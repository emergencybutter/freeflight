...
- View current, correctly-georeferenced FAA VFR and IFR charts, offline.
...
- Work fully offline once the current data cycle has been downloaded.
...
3. A pilot in the aircraft (phone/tablet in the cockpit, no data
   connection) wants offline access to the last-downloaded charts,
   procedures, and the weather briefing pulled before departure.
...
```
              ...
              ▼                       ▼                       ▼
     ┌────────────────┐     ┌─────────────────┐      ┌────────────────┐
     │  ff-api server  │     │   Web client     │      │ Android client │
     │  (axum, Rust)   │◄────┤  React/TS + WASM │      │ Kotlin/Compose │
     │  weather/NOTAM   │     │  core (ff-core)  │      │ + ff-core via  │
     │  proxy + cache   │     │  MapLibre GL JS  │      │ UniFFI + MapLibre│
     └────────────────┘     └─────────────────┘      └────────────────┘
```
...
- Compiled to **WebAssembly** (`wasm-bindgen`) for the web client, and to
  ...
  or ARINC 424 parsing code between platforms.
- UI and maps are platform-native: **MapLibre GL JS** on web, **MapLibre
  ...
  and even that can share a MapLibre style JSON.
- A small **`ff-api`** backend (axum) proxies weather/NOTAM APIs (adds
  ...
  endpoints) and serves the versioned cycle bundles produced by the ETL
  job. It does **not** hold client state — the clients are offline-first
  and own their local SQLite database.
...
freeflight/
  ...
  apps/
    web/                React + TypeScript, MapLibre GL JS, loads
                        ff-wasm, IndexedDB-backed SQLite (sql.js/wasm or
                        OPFS + sqlite-wasm) for offline cache
    android/            Kotlin, Jetpack Compose, MapLibre Android SDK,
                        loads ff-uniffi, on-device SQLite (via the same
                        ff-storage schema, driven through Rust)
  docs/
    DESIGN.md            (this file)
```
...
- Clients never talk to FAA/NOAA chart/procedure endpoints directly; they
  pull the pre-processed bundle from `ff-api`/CDN. This keeps client code
  simple, makes offline bundles reproducible, and isolates the app from
...
- Each client keeps a local SQLite database: on web via `sqlite-wasm` +
  OPFS (Origin Private File System) for durable storage; on Android via
  the same `ff-storage` schema opened directly with `rusqlite` inside the
  UniFFI core.
- On app start (or manually), the client checks `ff-api /cycles/latest`
  for a newer cycle id; if found, downloads the new SQLite + PMTiles
  bundle in the background (resumable, range-request friendly), verifies
...
- Weather/NOTAM are fetched opportunistically whenever online and cached
  locally with a "briefing snapshot" concept: the pilot explicitly takes
  a briefing before flight, which freezes the weather/NOTAM data used for
  planning even if it later goes stale, and time-stamps it clearly in the
  UI ("Briefing taken 45 min ago").
- Charts/procedures/airports work fully offline once a cycle is
  downloaded. Route planning works fully offline. Flight tracking (GPS
  logging) is inherently offline-capable. Only fetching a *new* briefing
  or a *new* cycle requires connectivity.
...
- MapLibre GL map with layer toggles: VFR sectional/TAC, IFR low/high
  enroute, and a vector overlay of airspace, airports, navaids drawn from
  the cycle SQLite (converted to GeoJSON on load).
...
| Client DB | SQLite (`rusqlite` native, `sqlite-wasm` in-browser) | single schema for server ETL and both clients; battle-tested offline story |
...
- **Offline-first**: every Phase 1 feature except live weather/NOTAM
  fetch and cycle updates must work with no network.
...
2. **d-TPP plate rendering**: FAA plates are PDF, not vector — rendering
   quality/perf on low-end Android devices needs a spike before
   committing to in-app PDF rendering vs. "open externally."
3. **ARINC 424 leg coding completeness**: implementing the full leg-type
